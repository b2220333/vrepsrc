...
#ifdef SIM_WITH_GUI
    if ( (!App::isFullScreen())&&(App::mainWindow!=NULL)&&(cmdIn->cmdId==ADD_PRIMITIVE_WITH_DLG_UITHREADCMD) )
    {
        CShape* shape=NULL;
        if (cmdIn->floatParams.size()>=3)
        {
            C3Vector s(&cmdIn->floatParams[0]);
            shape=CAddOperations::addPrimitive_withDialog(cmdIn->intParams[0],&s);
        }
        else
            shape=CAddOperations::addPrimitive_withDialog(cmdIn->intParams[0],NULL);
        if (shape!=NULL)
