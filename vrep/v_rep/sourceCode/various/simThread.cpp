...
void CSimThread::_executeSimulationThreadCommand(SSimulationThreadCommand cmd)
{
    ...
    if (cmd.sceneUniqueId==App::ct->environment->getSceneUniqueID())
    {
        ...
        if (cmd.cmdId==OPEN_MODAL_CUSTOMIZATION_SCRIPT_EDITOR_CMD)
        {
            ...
            {
                CLuaScriptObject* it=App::ct->luaScriptContainer->getScriptFromID(cmd.intParams[0]);
                if (it!=NULL)
                    it->killLuaState();
        ...
        {
            CFileOperations::loadModel(cmd.stringParams[0].c_str(),true,true,false,true,NULL,false);
...
//                    }
        ...
        if (cmd.cmdId==SET_SIZEFACTOR_COMMONPROPGUITRIGGEREDCMD)
        {
            C3DObject* it=App::ct->objCont->getObject(cmd.intParams[0]);
            if (it!=NULL)
                it->setSizeFactor(cmd.floatParams[0]);
        }
        if (cmd.cmdId==APPLY_GENERALPROP_COMMONPROPGUITRIGGEREDCMD)
        {
            ...
            if (last!=NULL)
            {
                ...
                for (size_t i=1;i<cmd.intParams.size();i++)
                {
                    ...
                    if (it!=NULL)
                    {
                        int objPropToCopy=sim_objectproperty_selectable|sim_objectproperty_selectmodelbaseinstead|sim_objectproperty_dontshowasinsidemodel|sim_objectproperty_selectinvisible|sim_objectproperty_depthinvisible;
        ...
        if (cmd.cmdId==TOGGLE_VISUALIZEEMMISIONS_ENVIRONMENTGUITRIGGEREDCMD)
        {
            App::ct->environment->setVisualizeWirelessEmitters(!App::ct->environment->getVisualizeWirelessEmitters());
        }
        if (cmd.cmdId==TOGGLE_VISUALIZERECEPTIONS_ENVIRONMENTGUITRIGGEREDCMD)
        {
        ...
        {
            if (cmd.boolParams[0])
                App::ct->environment->setEnableGeneralCallbackScript(true,cmd.stringParams[0].c_str());
            else
                App::ct->environment->setEnableGeneralCallbackScript(false,NULL);
        }
        ...
        if (cmd.cmdId==SET_PTSIZE_PTCLOUDGUITRIGGEREDCMD)
        {
            CPointCloud* it=App::ct->objCont->getPointCloud(cmd.intParams[0]);
            if (it!=NULL)
                it->setPointSize(cmd.floatParams[0]);
        }
            ...
            if ( (last!=NULL)&&((last->getJointMode()==sim_jointmode_force)||last->getHybridFunctionality()) )
            {
                ...
                for (size_t i=1;i<cmd.intParams.size();i++)
                {
                    ...
                    { // only when the two joints are in the same mode, or when both are in hybrid operation
                        ...
                        it->setEnableDynamicMotorCustomControl(false,NULL,NULL);
                        if (last->getEnableDynamicMotorCustomControl())
                            it->setEnableDynamicMotorCustomControl(true,callbackScriptText,&foldingInfo);
                    }
                }
            }
        }
        if (cmd.cmdId==SELECT_CUSTOMCTRL_JOINTDYNGUITRIGGEREDCMD)
        {
            ...
            if ((it!=NULL)&&(!it->getEnableDynamicMotorCustomControl()))
                it->setEnableDynamicMotorCustomControl(true,cmd.stringParams[0].c_str(),NULL);
        }
        ...
        if (cmd.cmdId==COMPUTE_MASSANDINERTIA_SHAPEDYNGUITRIGGEREDCMD)
        {
            for (size_t i=0;i<cmd.intParams.size();i++)
            {
                ...
                if ((it!=NULL)&&it->geomData->geomInfo->isConvex())
                {
                    ...
                    C3Vector com;
                    C3X3Matrix tensor;
                    float mass=CVolInt::getMassCenterOfMassAndInertiaTensor(&vert[0],vert.size()/3,&ind[0],ind.size()/3,cmd.floatParams[0],com,tensor);
                    C4Vector rot;
                    C3Vector pmoment;
                ...
                { // a bit more complicated here!
                    ...
                    if (it!=NULL)
                    {
                        if (it->getObjectType()==sim_object_joint_type)
                        {
                            ...
                            joint->setEnableDynamicMotorCustomControl(false,NULL,NULL);
                        }
                    }
                    else
        ...
        if (cmd.cmdId==INSERT_SCRIPT_SCRIPTGUITRIGGEREDCMD)
        {
            ...
            if (scriptT==sim_scripttype_contactcallback)
            {
                ...
                if (VFile::doesFileExist(filenameAndPath))
                {
                    ...
                    {
                        VFile::reportAndHandleFileExceptionError(e);
                        scriptInitText="Default collision/contact response script file could not be found!"; // do not use comments ("--"), we want to cause an execution error!
                    }
                }
                else
                    scriptInitText="Default collision/contact response script file could not be found!"; // do not use comments ("--"), we want to cause an execution error!
                newScriptID=App::ct->environment->setEnableCustomContactHandlingViaScript(true,scriptInitText.c_str());
            }
            if (scriptT==sim_scripttype_generalcallback)
            {
                ...
                if (VFile::doesFileExist(filenameAndPath))
                {
                    ...
                    {
                        VFile::reportAndHandleFileExceptionError(e);
                        scriptInitText="Default general callback script file could not be found!"; // do not use comments ("--"), we want to cause an execution error!
                    }
                }
                else
                    scriptInitText="Default general callback script file could not be found!"; // do not use comments ("--"), we want to cause an execution error!
                newScriptID=App::ct->environment->setEnableGeneralCallbackScript(true,scriptInitText.c_str());
            }
            if (scriptT==sim_scripttype_jointctrlcallback)
            {
                ...
                if (VFile::doesFileExist(filenameAndPath))
                {
                    ...
                    {
                        VFile::reportAndHandleFileExceptionError(e);
                        scriptInitText="Default joint custom control script file could not be found!"; // do not use comments ("--"), we want to cause an execution error!
                    }
                }
                else
                    scriptInitText="Default joint custom control script file could not be found!"; // do not use comments ("--"), we want to cause an execution error!
        ...
        if (cmd.cmdId==SET_ASSOCIATEDOBJECT_SCRIPTGUITRIGGEREDCMD)
        {
            ...
            if (it!=NULL)
            {
                ...
                {
                    CJoint* theOldJ=App::ct->objCont->getJoint(it->getObjectIDThatScriptIsAttachedTo_callback());
                    if (it2!=NULL)
                    {
                        // Check if the object doesn't already have a script attached:
                        if (App::ct->luaScriptContainer->getScriptFromObjectAttachedTo_callback(objID)==NULL)
                        {
                            ...
                            if (theNewJ!=NULL)
                            {
                                if (theOldJ!=NULL)
                                {
                                    it->setObjectIDThatScriptIsAttachedTo_callback(-1);
                                    theOldJ->setEnableDynamicMotorCustomControl(false,NULL,NULL);
                                }
                                theNewJ->setEnableDynamicMotorCustomControl(true,NULL,NULL);
                                it->setObjectIDThatScriptIsAttachedTo_callback(objID);
                            }
                        }
                    }
                    else
        ...
        if (cmd.cmdId==TOGGLE_DISABLEWHENNOTRUNNING_OBJECTMANIPGUITRIGGEREDCMD)
        {
            C3DObject* it=App::ct->objCont->getObject(cmd.intParams[0]);
            if (it!=NULL)
                it->setObjectManipulationDisabledDuringNonSimulation(!it->getObjectManipulationDisabledDuringNonSimulation());
        }
        if (cmd.cmdId==TOGGLE_DISABLEWHENRUNNING_OBJECTMANIPGUITRIGGEREDCMD)
        {
            C3DObject* it=App::ct->objCont->getObject(cmd.intParams[0]);
            if (it!=NULL)
                it->setObjectManipulationDisabledDuringSimulation(!it->getObjectManipulationDisabledDuringSimulation());
        }
        ...
        if (cmd.cmdId==SET_POSSTEPSIZE_OBJECTMANIPGUITRIGGEREDCMD)
        {
            ...
            if (it!=NULL)
            {
                ...
                {
                    if (App::ct->simulation->isSimulationStopped())
                        it->setObjectManipulationDisabledDuringNonSimulation(true);
                    else
                        it->setObjectManipulationDisabledDuringSimulation(true);
                }
                else
                {
                    if (App::ct->simulation->isSimulationStopped())
                        it->setObjectManipulationDisabledDuringNonSimulation(false);
                    else
                        it->setObjectManipulationDisabledDuringSimulation(false);
        ...
        if (cmd.cmdId==TOGGLE_ANTIALIASING_USERSETTINGSGUITRIGGEREDCMD)
        {
        ...
        if (cmd.cmdId==TOGGLE_TOGGLETRANSPARENTDLGS_USERSETTINGSGUITRIGGEREDCMD)
        {
        ...
        if (cmd.cmdId==TOGGLE_EXTERNALSCRIPTEDITOR_USERSETTINGSGUITRIGGEREDCMD)
        {
            App::userSettings->setUseBuiltInScriptEditorToSave(!App::userSettings->getUseBuiltInScriptEditorToSave());
            App::userSettings->saveUserSettings();
        }
        if (cmd.cmdId==SET_EXTERNALSCRIPTEDITOR_USERSETTINGSGUITRIGGEREDCMD)
        {
            std::string filenameAndPath(cmd.stringParams[0]);
            if (filenameAndPath.length()!=0)
                App::userSettings->externalScriptEditorToSave=filenameAndPath.c_str();
            App::userSettings->saveUserSettings();
        }
        if (cmd.cmdId==TOGGLE_THREADEDRENDERING_USERSETTINGSGUITRIGGEREDCMD)
        {
            ...
            if (App::userSettings->threadedRenderingDuringSimulation==1)
                App::uiThread->messageBox_warning(App::mainWindow,strTranslate(IDSN_THREADED_RENDERING),strTranslate(IDSN_THREADED_RENDERING_WARNING),VMESSAGEBOX_OKELI);
            App::userSettings->saveUserSettings();
        }
        ...
        if (cmd.cmdId==ADD_OBJECTTOSCENE_GUITRIGGEREDCMD)
        {
            if (cmd.intParams[0]==sim_object_path_type)
            {
                ...
                App::ct->objCont->addObjectToScene(it->copyYourself(),false);
...
{
    if ( (App::mainWindow!=NULL)&&App::ct->simulation->isSimulationStopped()&&(App::getEditModeType()==NO_EDIT_MODE) )
    {
        if (cmd.intParams[0]==0)
        { // Here we maybe need to load auto-saved scenes:
            ...
            { // ask what to do:
                if (!App::isFullScreen())
                {
                    ...
                    {
                        #ifdef MAC_VREP
                        if (VMESSAGEBOX_REPLY_YES==App::uiThread->messageBox_question(App::mainWindow,IDSN_VREP_CRASH,IDSN_VREP_CRASH_OR_NEW_INSTANCE_INFO_MAC,VMESSAGEBOX_YES_NO))
                        #else
