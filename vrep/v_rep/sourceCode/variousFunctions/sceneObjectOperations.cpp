...
{ // Return value is true if the command belonged to object edition menu and was executed
 // Can be called by the UI and SIM thread!
    ...
    /*
    if (commandID==SCENE_OBJECT_OPERATION_REPLACE_OBJECTS_SOOCMD)
    {
        if (!VThread::isCurrentThreadTheUiThread())
        { // we are NOT in the UI thread. We execute the command now:
            std::vector<int> sel;
            ...
            if (sel.size()>1)
            {
                App::addStatusbarMessage(IDSNS_REPLACING_OBJECTS);
                C3DObject* last=App::ct->objCont->getObject(sel[sel.size()-1]);
                for (int i=0;i<int(sel.size()-1);i++)
                {
                    C3DObject* it=App::ct->objCont->getObject(sel[i]);
                    ...
                    CLuaScriptObject* luaScript=App::ct->luaScriptContainer->getScriptFromObjectAttachedTo_child(last->getID());
                    if (luaScript!=NULL)
                    {
                        App::ct->luaScriptContainer->insertScript(luaScript->copyYourself());
                        luaScript->setObjectIDThatScriptIsAttachedTo_child(cop->getID());
                    }
                }
                POST_SCENE_CHANGED_ANNOUNCEMENT(""); // ************************** UNDO thingy **************************
                App::ct->objCont->deselectObjects(); // We clear selection
    ...
    if (commandID==SCENE_OBJECT_OPERATION_DECIMATE_SHAPE_SOOCMD)
    {
        ...
        { // we are NOT in the UI thread. We execute the command now:
            ...
            if (sh!=NULL)
            {
                ...
                cmdIn.intParams.push_back(ind.size()/3);
        ...
        { // we are NOT in the UI thread. We execute the command now:
            ...
            if (sh!=NULL)
            {
                ...
                if ((cmdOut.boolParams.size()>0)&&cmdOut.boolParams[0])
                {
                    ...
                    visionSensor->setViewAngle(60.0f*piValue/180.0f);
                    ...
                    App::ct->objCont->eraseObject(visionSensor);
                    App::ct->objCont->eraseObject(camera);
...
{ // CALL ONLY FROM THE MAIN SIMULATION THREAD!
    ...
    for (size_t i=0;i<objectsToErase.size();i++)
        ...
        if (individuallyConsiderMultishapeComponents&&(!it->geomData->geomInfo->isGeometric()))
        {
            ...
            for (int comp=0;comp<int(shapeComponents.size());comp++)
            {
                ...
                { // the convex decomposition routine sometimes fails producing good convectivity (i.e. there are slightly non-convex items that V-REP doesn't want to recognize as convex)
                    ...
                    CMeshRoutines::convexDecompose(&vert[0],vert.size(),&ind[0],ind.size(),outputVert,outputInd,
