...
{ // return value true means the hierarchy was refreshed
    ...
    if (rebuildHierarchyFlag)
        rebuildHierarchy();
    if (App::mainWindow->sceneHierarchyWidget!=NULL)
...
bool CHierarchy::leftMouseDblClick(int x,int y,int selectionStatus)
{
    ...
    if (scriptID!=-1)
    {
        ...
        if (it!=NULL)
        {
            ...
            if (it->getScriptType()==sim_scripttype_jointctrlcallback)
                auxData[0]=it->getObjectIDThatScriptIsAttachedTo_callback();
    ...
    if (App::getEditModeType()&(VERTEX_EDIT_MODE|PATH_EDIT_MODE))
    {
        // Did we double-click the icon?
        int objID=getActionObjectID_icon(mouseDownRelativePosition[0],mouseDownRelativePosition[1]);
        if (objID!=-9999)
