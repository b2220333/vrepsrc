...
void CQDlgCommonProperties::refresh()
{
    ...
    ui->qqSizeFactor->setEnabled(objIsSelected&&noEditModeNoSim);
    ...
    if (ls!=NULL)
    {
        ...
        ui->qqSizeFactor->setText(tt::getEString(false,ls->getSizeFactor(),3).c_str());
...
void CQDlgCommonProperties::on_qqSizeFactor_editingFinished()
{
    ...
    IF_UI_EVENT_CAN_READ_DATA
    {
        ...
        if (ok)
        {
            App::appendSimulationThreadCommand(SET_SIZEFACTOR_COMMONPROPGUITRIGGEREDCMD,App::ct->objCont->getLastSelectionID(),-1,newVal);
            App::appendSimulationThreadCommand(POST_SCENE_CHANGED_ANNOUNCEMENT_GUITRIGGEREDCMD);
        }
        App::appendSimulationThreadCommand(FULLREFRESH_ALL_DIALOGS_GUITRIGGEREDCMD);
    }
