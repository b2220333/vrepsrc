...
void CShapeEditMode::paste()
{
    ...
    {
        deselectEditModeBuffer();
        int startInd=_editionVertices.size()/3;
        for (int i=0;i<int(editBufferVerticesCopy.size());i++)
            _editionVertices.push_back(editBufferVerticesCopy[i]);
        int orSize=_editionIndices.size()/3;
        for (int i=0;i<int(editBufferIndicesCopy.size())/3;i++)
        {
            int ind[3];
            ...
            _editionIndicesToEdgesIndex.push_back(index);
            addItemToEditModeBuffer(orSize+i,true);
        }
        for (int i=0;i<int(editBufferNormalsCopy.size());i++)
            _editionNormals.push_back(editBufferNormalsCopy[i]);
        for (int i=0;i<int(editBufferTextureCoordsCopy.size());i++)
...
void CShapeEditMode::addMenu(VMenu* menu)
{
    ...
    int buffSize=editBufferVerticesCopy.size();
