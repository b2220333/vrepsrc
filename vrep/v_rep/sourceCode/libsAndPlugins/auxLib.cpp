...
bool CAuxLib::loadLibrary()
{
    ...
    {
        if (!_getVrepAuxLibProcAddresses())
        { // Error
            unloadLibrary();
...
bool CAuxLib::_loadLibrary(const char* pathAndFilename)
{
...
WLibraryFunc CAuxLib::_getProcAddress(const char* funcName)
{
    return VVarious::resolveLibraryFuncName(_lib,funcName);
}

bool CAuxLib::_getVrepAuxLibProcAddresses()
{
