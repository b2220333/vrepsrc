...
{   // textureCoords can be NULL, in which case texCoordBufferId can also be NULL
    ...
    if (theVertexBuff==NULL)
    {
        ...
        vertexBufferId[0]=_buildVertexBuffer(&individualVertices[0],individualVertices.size()/3);
...
bool CGlBufferObjects::drawEdges(const float* vertices,int verticesCnt,const int* indices,int indicesCnt,const unsigned char* edges,int* edgeBufferId)
{
    ...
    if (theEdgeBuff==NULL)
    {
        ...
        edgeBufferId[0]=_buildEdgeBuffer(&individualVertices[0],individualVertices.size()/3);
