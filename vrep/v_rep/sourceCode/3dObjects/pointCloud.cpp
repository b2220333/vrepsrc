...
void CPointCloud::subtractPointCloud(const CPointCloud* pointCloud,float distanceTolerance)
{
    ...
    if (pointCloud->getPointCloudInfo()!=NULL)
    {
        ...
        removePoints(&pts[0],pts.size()/3,false,distanceTolerance);
    }
}
...
void CPointCloud::insertShape(const CShape* shape)
{
    ...
    insertPoints(&pts[0],pts.size()/3,true,NULL,false);
}
...
void CPointCloud::insertOctree(const COctree* octree)
{
    ...
    if (octree->getOctreeInfo()!=NULL)
    {
        ...
        insertPoints(&pts[0],pts.size()/3,false,NULL,false);
    }
}
...
void CPointCloud::insertPointCloud(const CPointCloud* pointCloud)
{
    ...
    insertPoints(&pts[0],pts.size()/3,false,&_cols[0],true);
}
...
void CPointCloud::setDoNotUseCalculationStructure(bool s)
{
    if (s!=_doNotUseOctreeStructure)
    {
        ...
        if (_points.size()>0)
        {
            ...
            clear();
            insertPoints(&p[0],p.size()/3,true,&c[0],true);
        }
    }
}
...
void CPointCloud::performGroupLoadingMapping(std::vector<int>* map)
