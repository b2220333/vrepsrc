...
void CGeomProxy::initializeCalculationStructureIfNeeded()
{
    ...
    if ((collInfo==NULL)&&(geomInfo!=NULL))
    {
        ...
        collInfo=CPluginContainer::mesh_createCollisionInformationStructure(&wvert[0],wvert.size(),&wind[0],wind.size(),maxTriSize,geomInfo->getEdgeThresholdAngle(),App::userSettings->triCountInOBB);
    }
}
        ...
        while (theName.compare(SER_END_OF_OBJECT)!=0)
        {
            ...
            if (theName.compare(SER_END_OF_OBJECT)!=0)
            {
                ...
                if (theName.compare("Coi")==0)
                {
                    ...
                    std::vector<int> wind;
                    geomInfo->getCumulativeMeshes(wvert,&wind,NULL);
                    collInfo=CPluginContainer::mesh_getCollisionInformationStructureFromSerializationData(&data[0],&wvert[0],wvert.size(),&wind[0],wind.size());
                }
                if (noHit)
                    ar.loadUnknownData();
            }
