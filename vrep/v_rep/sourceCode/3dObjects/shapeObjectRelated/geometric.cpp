...
{ // function has virtual/non-virtual counterpart!
    if (!_wireframe)
    {
        ...
        void* data[40];
        data[0]=&_vertices[0];
        int vs=_vertices.size()/3;
        data[1]=&vs;
        data[2]=&_indices[0];
        int is=_indices.size()/3;
        data[3]=&is;
        data[4]=&_normals[0];
        int ns=_normals.size()/3;
        data[5]=&ns;
        data[6]=tr2.X.data;
...
//      data[7]=euler.data;
        ...
        if (tp!=NULL)
        {
            ...
            int texCoordSize=textureCoords->size()/2;
...
void CGeometric::_computeVisibleEdges()
{
    ...
    _edges.clear();
    std::vector<int> eIDs;
    CMeshRoutines::getEdgeFeatures(&_vertices[0],_vertices.size(),&_indices[0],_indices.size(),NULL,&eIDs,NULL,softAngle,true,_hideEdgeBorders);
