...
void CGraph::drawGrid(int windowSize[2],float graphPosition[2],float graphSize[2])
{
    ...
    if (graphGrid)
    {
        ...
        ogl::drawRandom2dLines(&ogl::buffer[0],ogl::buffer.size()/2,false,0.0f);
        ogl::buffer.clear();
        ...
        for (int i=0;i<int(daten.size());i++)
        {
            if (daten[i]->getVisible())
            {
                ...
                { // We display that curve
                    ...
                    {
                        if (it->getLinkPoints())
                            ogl::drawRandom2dLines(&ogl::buffer[0],ogl::buffer.size()/2,true,0.0f);
                        else
                            ogl::drawRandom2dPoints(&ogl::buffer[0],ogl::buffer.size()/2,0.0f);
                    }
                    ogl::buffer.clear();
                    glPointSize(1.0f);
                    glLineWidth(1.0f);
