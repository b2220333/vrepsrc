...
void _drawPoints(CDrawingObject* drawingObject,int displayAttrib,const C4X4Matrix& cameraRTM,const float normalVectorForLinesAndPoints[3])
{
    ...
    { // This is a special routine in case we want various point sizes (change not possible between glBegin and glEnd)
        ...
        {
            if (glCols.size()==0)
                ogl::drawRandom3dPointsEx(&ogl::buffer[0],ogl::buffer.size()/3,NULL,NULL,&glSizes[0],_objectType&(sim_drawing_emissioncolor|sim_drawing_auxchannelcolor2),normalVectorForLinesAndPoints);
            else
                ogl::drawRandom3dPointsEx(&ogl::buffer[0],ogl::buffer.size()/3,NULL,&glCols[0],&glSizes[0],_objectType&(sim_drawing_emissioncolor|sim_drawing_auxchannelcolor2),normalVectorForLinesAndPoints);
        }
        ogl::buffer.clear();
        glPointSize(1.0f);
    }
    ...
    { // fixed point size
        ...
        {
            if (glCols.size()==0)
                ogl::drawRandom3dPointsEx(&ogl::buffer[0],ogl::buffer.size()/3,NULL,NULL,NULL,_objectType&(sim_drawing_emissioncolor|sim_drawing_auxchannelcolor2),normalVectorForLinesAndPoints);
            else
                ogl::drawRandom3dPointsEx(&ogl::buffer[0],ogl::buffer.size()/3,NULL,&glCols[0],NULL,_objectType&(sim_drawing_emissioncolor|sim_drawing_auxchannelcolor2),normalVectorForLinesAndPoints);
        }
        ogl::buffer.clear();
        glPointSize(1.0f);
    }
