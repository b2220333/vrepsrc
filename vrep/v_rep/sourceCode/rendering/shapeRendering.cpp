...
void _displayInertia(CGeomWrap* geomWrap,float bboxDiagonal,const float normalVectorForPointsAndLines[3])
{
    ...
    ogl::addBuffer3DPoints(-ma(0)*mf,+ma(1)*mf,-ma(2)*mf);
    ogl::addBuffer3DPoints(-ma(0)*mf,+ma(1)*mf,+ma(2)*mf);
    ogl::drawRandom3dLines(&ogl::buffer[0],ogl::buffer.size()/3,true,normalVectorForPointsAndLines);
    ogl::buffer.clear();
    ogl::addBuffer3DPoints(-ma(0)*mf,-ma(1)*mf,-ma(2)*mf);
    ogl::addBuffer3DPoints(-ma(0)*mf,+ma(1)*mf,-ma(2)*mf);
    ogl::addBuffer3DPoints(+ma(0)*mf,-ma(1)*mf,-ma(2)*mf);
    ...
    ogl::addBuffer3DPoints(+ma(0)*mf,-ma(1)*mf,+ma(2)*mf);
    ogl::addBuffer3DPoints(+ma(0)*mf,+ma(1)*mf,+ma(2)*mf);
    ogl::drawRandom3dLines(&ogl::buffer[0],ogl::buffer.size()/3,false,normalVectorForPointsAndLines);
    ogl::buffer.clear();
...
void _displayTriangles(CGeometric* geometric,int geomModifCounter,CTextureProperty* tp)
{
    ...
    if (tp!=NULL)
    {
        ...
        _drawTriangles(&_vertices[0],_vertices.size()/3,&_indices[0],_indices.size(),&_normals[0],&textureCoords->at(0),geometric->getVertexBufferIdPtr(),geometric->getNormalBufferIdPtr(),texCoordBufferIdPtr);
        _end3DTextureDisplay(tp);
    }
    else
        _drawTriangles(&_vertices[0],_vertices.size()/3,&_indices[0],_indices.size(),&_normals[0],NULL,geometric->getVertexBufferIdPtr(),geometric->getNormalBufferIdPtr(),NULL);
}
...
void displayGeometric(CGeometric* geometric,CGeomProxy* geomData,int displayAttrib,CVisualParam* collisionColor,int dynObjFlag_forVisualization,int transparencyHandling,bool multishapeEditSelected)
{
    ...
    if ((!meantAsWire)||((displayAttrib&sim_displayattribute_forvisionsensor)==0)) // added on 2010/08/11 because the wireframe part of the default floor would be rendered solid
    {
        ...
        if ( (edges||wire)&&((displayAttrib&sim_displayattribute_trianglewireframe)==0)&&(!nonPureDynamicDisplay) )
        {
            ...
            bool nothingDisplayed=(!_drawEdges(&_vertices[0],_vertices.size()/3,&_indices[0],_indices.size(),&_edges[0],geometric->getEdgeBufferIdPtr()));
...
void displayGeometric_colorCoded(CGeometric* geometric,CGeomProxy* geomData,int objectId,int displayAttrib)
{
    ...
    if ((displayAttrib&sim_displayattribute_colorcodedtriangles)!=0)
    {
        ...
        _drawColorCodedTriangles(&_vertices[0],_vertices.size()/3,&_indices[0],_indices.size(),&_normals[0],geometric->getVertexBufferIdPtr(),geometric->getNormalBufferIdPtr());
    }
    else
        _displayTriangles(geometric,geomData->getGeomDataModificationCounter(),NULL);
    glDisable(GL_CULL_FACE);
