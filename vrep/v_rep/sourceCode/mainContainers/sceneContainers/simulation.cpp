...
void CSimulation::advanceSimulationByOneStep()
{
    ...
    if (_automaticPauseEnabled&&(_simulationTime_ns>=_simulationTimeToPause_ns))
    {
        pauseSimulation();
        _automaticPauseEnabled=false;
    }
    ...
    if (index==0)
    {
        ...
        if (_realTimeSimulation)
        {
            ...
            if (getSimulationTimeHistoryDurations_ns(st_,str_))
            {
                ...
                {
                    if (abs((long long int)(_simulationTime_ns-simulationTime_real_ns))>10*getSimulationTimeStep_speedModified_ns())
...
void CSimulation::addMenu(VMenu* menu)
{
    ...
    if (isSimulationPaused())
        ...
        menu->appendMenuItem(App::mainWindow->getPlayViaGuiEnabled()&&noEditMode&&(!isSimulationRunning()),false,SIMULATION_COMMANDS_START_RESUME_SIMULATION_REQUEST_SCCMD,IDS_START_SIMULATION_MENU_ITEM);
    menu->appendMenuItem(App::mainWindow->getPauseViaGuiEnabled()&&noEditMode&&isSimulationRunning(),false,SIMULATION_COMMANDS_PAUSE_SIMULATION_REQUEST_SCCMD,IDS_PAUSE_SIMULATION_MENU_ITEM);
    menu->appendMenuItem(App::mainWindow->getStopViaGuiEnabled()&&noEditMode&&(!isSimulationStopped()),false,SIMULATION_COMMANDS_STOP_SIMULATION_REQUEST_SCCMD,IDS_STOP_SIMULATION_MENU_ITEM);
    menu->appendMenuSeparator();
    int version;
    int engine=App::ct->dynamicsContainer->getDynamicEngineType(&version);
    menu->appendMenuItem(noEditMode&&isSimulationStopped(),(engine==sim_physics_bullet)&&(version==0),SIMULATION_COMMANDS_TOGGLE_TO_BULLET_2_78_ENGINE_SCCMD,IDS_SWITCH_TO_BULLET_2_78_ENGINE_MENU_ITEM,true);
    menu->appendMenuItem(noEditMode&&isSimulationStopped(),(engine==sim_physics_bullet)&&(version==283),SIMULATION_COMMANDS_TOGGLE_TO_BULLET_2_83_ENGINE_SCCMD,IDS_SWITCH_TO_BULLET_2_83_ENGINE_MENU_ITEM,true);
    menu->appendMenuItem(noEditMode&&isSimulationStopped(),engine==sim_physics_ode,SIMULATION_COMMANDS_TOGGLE_TO_ODE_ENGINE_SCCMD,IDS_SWITCH_TO_ODE_ENGINE_MENU_ITEM,true);
    menu->appendMenuItem(noEditMode&&isSimulationStopped(),engine==sim_physics_vortex,SIMULATION_COMMANDS_TOGGLE_TO_VORTEX_ENGINE_SCCMD,IDS_SWITCH_TO_VORTEX_ENGINE_MENU_ITEM,true);
    menu->appendMenuItem(noEditMode&&isSimulationStopped(),engine==sim_physics_newton,SIMULATION_COMMANDS_TOGGLE_TO_NEWTON_ENGINE_SCCMD,IDS_SWITCH_TO_NEWTON_ENGINE_MENU_ITEM,true);
    menu->appendMenuSeparator();
    menu->appendMenuItem(noEditMode&&isSimulationStopped(),App::ct->simulation->getRealTimeSimulation(),SIMULATION_COMMANDS_TOGGLE_REAL_TIME_SIMULATION_SCCMD,IDSN_REAL_TIME_SIMULATION,true);
    ...
    menu->appendMenuItem(isSimulationRunning()&&(!App::mainWindow->oglSurface->isSceneSelectionActive()||App::mainWindow->oglSurface->isPageSelectionActive()||App::mainWindow->oglSurface->isViewSelectionActive()),!App::mainWindow->getOpenGlDisplayEnabled(),SIMULATION_COMMANDS_TOGGLE_VISUALIZATION_SCCMD,IDSN_TOGGLE_VISUALIZATION,true);
