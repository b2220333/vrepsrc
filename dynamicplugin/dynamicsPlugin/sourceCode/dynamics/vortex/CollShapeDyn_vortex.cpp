...
//  const int materialUniqueId=intParams[7];
    ...
    { // We have a pure primitive here. It is either a simple collision objects, or a compound of simple collision objects.
        ...
        { // We a have a pure MULTISHAPE!! Here we have a compound of simple collision objects
            ...
            { // we go through all individual collision object components and add them to a compound collision object:
                ...
                for (int j=0;j<loopCnt;j++)
                {
                    ...
                    Vx::VxGeometry* vortexGeom = _createVortexSimpleGeometry(pType, s,hollowScaling, NULL, 1.0);
        ...
        { // we have a SIMPLE pure shape. This is a single simple collision object (e.g. sphere, cuboid, cylinder or heightfield)
            ...
            Vx::VxCompositeCollisionGeometry* vortexCompositeCollisionGeom=NULL;
        ...
        {   // We have a general-type geom object (trimesh)
            ...
            { // We a have a grouping of random meshes.
                ...
                for (int comp=0;comp<componentListSize;comp++)
                {
                    ...
                    Vx::VxGeometry* vortexGeometry = NULL;
...
CCollShapeDyn_vortex::~CCollShapeDyn_vortex()
{
    ...
    {
        if (_vortexGeoms[i]->getPart() != NULL)
        {
...
Vx::VxTriangleMeshUVGrid* CCollShapeDyn_vortex::_createVortexUVGridMesh(float* allVertices, int allVerticesSize, int* allIndices, int allIndicesSize, float linScaling)
{
    ...
    */
    ...
    { // We need to scale the vertices
        ...
        bbox.expandBy(vortexVertices_scaled[i]);
    }
    ...
    // Could return NULL and do a BVTree instead.
...
//  const int autoSleepStepLiveThreshold=intParams[6];
    const int materialUniqueId=intParams[7];
//*
    std::string materialString("VREP");
    materialString+=boost::lexical_cast<std::string>(materialUniqueId);

    // studiochange
    Vx::VxSmartPtr<Vx::VxRigidBodyResponseModel> respModel=universe->getRigidBodyResponseModel();

    // studiochange
    Vx::VxMaterial* material=respModel->getMaterialTable()->getMaterial(materialString.c_str());
    //    Vx::VxMaterial* material = universe->getMaterial(materialString.c_str());
    ...
    if (material==NULL)
    {
        ...
        //material = universe->getMaterialTable()->registerMaterial(materialString.c_str());
